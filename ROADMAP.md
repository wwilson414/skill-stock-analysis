...
- [o] **4-5a** 仓位管理：单只股票最大仓位 20%（`RISK.max_per_stock`），单一 phase 最大仓位 60%（`RISK.max_per_phase`）→ `risk_monitor.py` `can_enter()` ✅ **类已实现（+单测）；⚠️ 未接入 paper_trader 引擎（入场限制在回放中未生效）**
- [o] **4-5b** 止损：个股 -8% 止损（`RISK.stop_loss_pct`），组合 -15% 清仓（`RISK.circuit_breaker_pct`）→ `risk_monitor.py` `check_stop_loss()` / `check_circuit_breaker()` ✅ **类已实现（+单测）；⚠️ 未接入 paper_trader 引擎（止损/熔断在回放中未生效）**
- [o] **4-5c** 日终监控：`risk_monitor.py` `eod_report()` 每日输出持仓 + 风险敞口 + 异常信号告警 ✅ **函数已实现；⚠️ 未在回放日循环中调用输出**

> 状态记法：`[o]` = 部分完成（功能可用但未集成/验收）。详见 HANDOFF §7.5 未完成项清单。

### P4 验收标准

| 指标 | 目标 | 说明 | 当前状态 |
|---|---|---|---|
| 组合 Sharpe | > 0.08 | 样本内（2022-2025） | ⚠️ net 0.059 / open 0.086（未达标，已记账 4-4 调优） |
| 样本外 Sharpe | > 0.3 | 2025-09 ~ 2026-09 | ⚠️ **未跑**（4-4c 待执行） |
| max_dd | < 30% | 组合层面 | ⚠️ **未跑**—样本内 net 74.8% 远超目标，4-4c 需验证 |
| 盈亏比 | > 1.5 | 平均盈利/平均亏损 | ❌ **指标未实现**（`_performance()` 无此输出） |
| 信号覆盖率 | > 60% | 每日至少 1 只股票有信号 | ❌ **指标未实现**（无统计） |

> 两个 ❌ 指标需先实现再跑验收；详见 HANDOFF §7.5。
...
- **止损/熔断未实证**：RiskMonitor 未接入引擎，止损/熔断对 Sharpe/max_dd 的实际影响未知（接入后可能显著改善 max_dd）
