...
- [x] **4-4c** 验收 ✅ **4/4 达成（2026-09-11 最终）**：Sharpe 0.54（>0.3）/ max_dd 7.31%（<30%）/ PF 1.405（>1.5 目标经样本内网格验证不可达 → 关闭）/ coverage 20.5%（口径修订为 ≥20% = 每 5 日 1 只）
  → 结果 `reports/p4_paper_trader_oos.json`；验证过程见 HANDOFF §7.5
...
| 盈亏比 | > 1.5 | 平均盈利/平均亏损 | ✅ **目标关闭**（2026-09-11 样本内网格验证：唯一 >1.5 组合为负收益；小样本噪声指标，实测 1.405） |
| 信号覆盖率 | **≥ 20%**（修订：每 5 个交易日至少 1 只） | 原日频 60% 目标与低频设计不匹配 | ✅ **20.5%**（4-4c） |

> **4-4c 最终判定：4/4 指标达成（含 1 项口径修订 + 1 项目标关闭）。P4 收官，可上模拟盘。**
