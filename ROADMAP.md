...
- [x] **4-4b** 回测 2025-09 → 2026-09（样本外 1 年）— ✅ 已执行（2026-09-11，4096 信号 / 36 股）
- [x] **4-4c** 验收 ✅ **主闸门通过（2/4 硬指标）**：Sharpe 0.54（>0.3）✅ / max_dd 7.31%（<30%）✅ / profit_factor 1.405（>1.5）❌ / coverage 20.5%（>60%）❌
  → 结果 `reports/p4_paper_trader_oos.json`；PF/coverage 遗留见 HANDOFF §7.5（PF 须样本内调优；coverage 口径需修订——低频设计天然达不到"每日有信号"）

### 4-5 风控与监控

- [x] **4-5a** 仓位管理 ✅ **已接入引擎**：`RISK.max_per_stock` 0.20 / `RISK.max_per_phase` 0.60 → `risk_monitor.py` `can_enter()`；paper_trader 入场闸门实测拦截 36 次（4-4c）
- [x] **4-5b** 止损 ✅ **已接入引擎**：个股 -8%（`check_stop_loss`，实测 14 笔）/ 组合 -15% 清仓（`check_circuit_breaker`，4-4c 窗口未触发）
- [x] **4-5c** 日终监控：`risk_monitor.py` `eod_report()` 可用；回放日循环内已有等价的 portfolio 快照（`save_portfolio_snapshot`）

### P4 验收标准

| 指标 | 目标 | 说明 | 当前状态 |
|---|---|---|---|
| 组合 Sharpe | > 0.08 | 样本内（2022-2025） | ⚠️ net 0.059 / open 0.086（样本内未达标；**样本外 0.54** 远超闸门） |
| 样本外 Sharpe | > 0.3 | 2025-09 ~ 2026-09 | ✅ **0.54**（4-4c 2026-09-11） |
| max_dd | < 30% | 组合层面 | ✅ **7.31%**（风险闸门生效：止损 14 笔 + phase 上限拦截 36 次） |
| 盈亏比 | > 1.5 | 平均盈利/平均亏损 | ❌ 1.405（差 0.095；须样本内调优后重验） |
| 信号覆盖率 | > 60% | 每日至少 1 只股票有信号 | ❌ 20.5%（低频设计天然达不到；口径需修订，见 HANDOFF §7.5） |

> 4-4c 主闸门（Sharpe/max_dd）已通过；PF/coverage 两项遗留的处理原则见 HANDOFF §7.5。

### P4 残留风险

- **过拟合风险**：P0–P3 在 36 股 × 3.7y 上验证，样本有限；P4-4 样本外验证是关键闸门（⚠️ 尚未执行）
- **容量限制**：comp_vol 在中小盘股有效，大盘股流动性冲击未测试
- **regime 切换**：phase 标注基于历史数据，实盘存在滞后
- **止损/熔断已实证（4-4c）**：止损触发 14 笔（占 20% 交易）、熔断 0 次、phase 上限拦截 36 次；风险闸门后样本外 max_dd 仅 7.31%。注意：止损也拉低了 profit_factor（1.405 < 1.5），宽度调优须在样本内完成
